...
vc.nc.commit_list(statements=[
    ...
    "USING PERIODIC COMMIT 500 LOAD CSV WITH HEADERS FROM 'file:///swc.tsv' AS row FIELDTERMINATOR '\\t' MATCH (s:Individual {short_form: row.query}), (b:Individual {short_form: row.target}) MERGE (s)-[r:has_similar_morphology_to { iri: 'http://n2o.neo/custom/has_similar_morphology_to', short_form: 'has_similar_morphology_to', type: 'Annotation' }]->(b) SET r.NBLAST_score = [toFloat(row.score)], r.mirrored = CASE WHEN row.mirrored = 'y' THEN true ELSE false END SET s:NBLAST, b:NBLAST",
    "USING PERIODIC COMMIT 500 LOAD CSV WITH HEADERS FROM 'file:///hb_to_hb_ol_fw_fc_swc_swc.tsv' AS row FIELDTERMINATOR '\\t' MATCH (s:Individual {short_form: row.query}), (b:Individual {short_form: row.target}) MERGE (s)-[r:has_similar_morphology_to { iri: 'http://n2o.neo/custom/has_similar_morphology_to', short_form: 'has_similar_morphology_to', type: 'Annotation' }]->(b) SET r.NBLAST_score = [toFloat(row.score)], r.mirrored = CASE WHEN row.mirrored = 'y' THEN true ELSE false END SET s:NBLAST, b:NBLAST"
])
stop = timeit.default_timer()
print('Run time: ', stop - start)
